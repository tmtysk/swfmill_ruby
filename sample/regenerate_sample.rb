...
swf = SwfmillRuby::Swf.parseXml(File.open("data/sample_template.xml").read, true)

# initialize template movieclip by preserved xml generated by DefineSprite#partialize
# avoid analysing swf's structure using "template_mode".
animation = SwfmillRuby::DefineSprite.parseXml(File.open("data/animation_partial.xml").read, true)
...
File.open("data/regenerated_gsub.swf", "w") do |f|
  f.puts SwfmillRuby::Swfmill.xml2swf(File.open("data/animation_template_gsub.xml").read.gsub(/####PARTIAL_MOVIECLIP_5####/, File.open("data/animation_partial_gsub.xml").read))
end
